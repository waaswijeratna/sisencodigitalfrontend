...
export default function KineticGrid({
  ...
  // ── Setup ───────────────────────────────────────────────────────────────────
  // NOTE: everything below is now scoped to the CONTAINER, not the window.
  // That's the actual bug fix — the canvas used to size itself off
  // window.innerWidth/innerHeight and used `fixed` positioning, so no
  // parent width (like w-[50vw]) could ever constrain it.
  ...
  useEffect(() => {
    ...
    // Listen on the container, not window, so coordinates are local to it
